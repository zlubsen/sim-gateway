...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (to_rt_tx, to_rt_rx) = tokio::sync::mpsc::channel(10);
    let (to_gui_tx, to_gui_rx) = std::sync::mpsc::channel();
            ...
            loop {
                if event::poll(poll_rate).expect("Key event polling error") {
                    if let CEvent::Key(key) = event::read().expect("Key event read error") {
                        if mode == Mode::Interactive {
                            to_gui_tx.send(key).expect("Send key event to GUI error");
                        }
                        to_rt_tx.blocking_send(key).expect("Send key event to runtime error");
                    }
                }
    ...
    let gui_thread = match config.mode {
        Mode::Interactive => { start_gui(Settings {
            placeholder: 0
            }, to_gui_rx)
        }
    ...
    let _runtime_thread = {
        ...
        rt_builder.spawn(move || {
            runtime.block_on(start_main_task(to_rt_rx, shutdown_tx));
            runtime.shutdown_background();
        })
    };

    if _runtime_thread.is_ok() {
        _runtime_thread.unwrap().join().expect("Couldn't join on the associated thread, Runtime");
    }
    if Mode::Interactive == config.mode {
        if let Ok(gui) = gui_thread {
            gui.join().expect("Couldn't join on the associated thread, GUI");
        }
    }
    if let Ok(input) = input_thread {
        input.join().expect("Couldn't join on the associated thread, Input");
    }
